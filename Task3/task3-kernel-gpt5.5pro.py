...
GROUP_SIZE = 128
_GRAD_BLOCK_M = 8       # keeps max grid size below 65535 for Ascend-style backends
_Y_BLOCK_M = 128        # fixed K-group size over tokens
_Y_BLOCK_H = 16         # moderate tile size for cross-platform UB/register pressure
...
def _silu_grad_quant_kernel(
    ...
    """Compute d_gate and d_up for one [BLOCK_M, 128] hidden group and quantize per row."""
    ...
    # Reference computes gradients/y in fp32 and quantizes the bf16-rounded values.
...
def _silu_y_t_quant_kernel(
    ...
    """Recompute y = silu(gate) * up, transpose it, and quantize per 128-token group."""
    ...
    # y tile is logically [128 tokens, BLOCK_H hidden channels].  This layout
    # gives a per-hidden-channel reduction over the 128-token K-group.
    ...
    # Store transposed: output layout is y_q_t[H, M].
...
def silu_dot_fwd_bwd_quant_fuse(
    ...
    """
    Fused SiLU-dot recompute, SwiGLU backward, and INT8 quantization.

    Expected workload shapes:
      x            : [M, 2H], bf16, contiguous, packed as [gate, up]
      grad_y       : [M, H], bf16, contiguous
    ...
    # The official workloads satisfy these constraints.  Keeping the kernels
    # mask-free improves portability and throughput on the required platforms.
    assert two_h == H * 2

...
def _w4a8_group_gemm_moe_kernel(
    ...
    for group_id in range(0, NUM_GROUPS):
        ...
        x_even = tl.load(x_base[:, None] + k_even[None, :] * stride_x_k, mask=mask_m[:, None], other=0)
        x_odd = tl.load(x_base[:, None] + k_odd[None, :] * stride_x_k, mask=mask_m[:, None], other=0)

        w_packed = tl.load(w_base + pk_offsets[:, None] * stride_w_k + w_n_offsets[None, :], mask=mask_n[None, :], other=0)
        w_zero_vals = tl.load(wz_base + wz_n_offsets + group_id * stride_wz_g, mask=mask_n, other=0)
        ...
        partial = tl.dot(x_even.to(tl.int8), w_low) + tl.dot(x_odd.to(tl.int8), w_high)
        w_scale_vals = tl.load(ws_base + ws_n_offsets + group_id * stride_ws_g, mask=mask_n, other=0.0)

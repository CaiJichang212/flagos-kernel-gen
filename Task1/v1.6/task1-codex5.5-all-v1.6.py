...
@triton.jit
def _w4a8_group_gemm_moe_kernel_ascend_transposed(
    x_q_ptr,
    x_scale_ptr,
    w_q4_packed_ptr,
    w_scale_ptr,
    ...
    for group_id in range(0, K // GROUP_SIZE):
        ...
        x_even = tl.load(
            x_q_ptr + rows[:, None] * stride_x_m + k_even[None, :] * stride_x_k,
            mask=mask_m[:, None],
            other=0,
        )
        x_odd = tl.load(
            x_q_ptr + rows[:, None] * stride_x_m + k_odd[None, :] * stride_x_k,
            mask=mask_m[:, None],
            other=0,
        )
        ...
        partial = tl.dot(x_even.to(tl.int8), tl.trans(w_low)) + tl.dot(
            x_odd.to(tl.int8), tl.trans(w_high)
        )
...
def _run_ascend(
    ...
    _w4a8_group_gemm_moe_kernel_ascend_transposed[grid](
        x_q,
        x_scale,
        w_q4_packed,
        w_scale,

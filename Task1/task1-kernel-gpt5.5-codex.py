...
def w4a8_group_gemm_moe(
    ...
    """
    W4A8 grouped GEMM for MoE inference.

    The kernel consumes packed INT4 weights directly. For each quantization
    group it forms even/odd INT8 operands from the low/high nibbles, applies
    the per-group zero point, accumulates the two INT8 dot products, and then
    applies weight and activation scales before writing bf16 output.
    """
    M_total, K = x_q.shape

...
def _w4a8_group_gemm_moe_kernel(
    ...
    group_id = remainder // num_pid_in_group
    first_pid_m = group_id * GROUP_TILES
...
@triton.jit
def _w4a8_group_gemm_moe_kernel_large(
    x_q_ptr,
    x_scale_ptr,
    w_q4_packed_ptr,
    w_scale_ptr,
    ...
    num_pid_n = num_tiles_n
    num_pid_in_group = GROUP_TILES * num_pid_n
    group_id_tile = remainder // num_pid_in_group
    first_pid_m = group_id_tile * GROUP_TILES
    group_size_m = tl.minimum(num_tiles_m - first_pid_m, GROUP_TILES)
    pid_m = first_pid_m + (remainder % group_size_m)
    pid_n = (remainder % num_pid_in_group) // group_size_m
...
def w4a8_group_gemm_moe(
    ...
    is_large = (N >= 1408 and max_m_per_expert >= 128) or (N >= 2048)
    ...
    grid = (total_programs,)
    kernel_fn = _w4a8_group_gemm_moe_kernel_large if is_large else _w4a8_group_gemm_moe_kernel

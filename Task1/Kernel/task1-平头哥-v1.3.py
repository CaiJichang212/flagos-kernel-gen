...
def _w4a8_group_gemm_moe_kernel(
    ...
    x_base = x_q_ptr + rows[:, None].to(tl.int64) * stride_x_m
    w_base = w_q4_packed_ptr + pid_e.to(tl.int64) * stride_w_e
    ws_base = w_scale_ptr + pid_e.to(tl.int64) * stride_ws_e
    wz_base = w_zero_ptr + pid_e.to(tl.int64) * stride_wz_e
    ...
    for group_id in range(0, NUM_GROUPS):
        ...
        k_base = group_id * GROUP_SIZE

        pk_offsets = pk_base + offs_pk
        k_even = k_base + offs_pk * 2
        k_odd = k_even + 1
    ...
    out_ptrs = (
        out_ptr
        + rows[:, None].to(tl.int64) * stride_out_m
...
def w4a8_group_gemm_moe(
    ...
    nw = 8 if block_n >= 128 else 4
    ns = 3 if num_groups >= 4 else 2
